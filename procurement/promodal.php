...
if (isset($_POST['dateup'])) {
    ...
    $updateQuery = "UPDATE tbl_inout SET ActionTaken = '$action', Remarks = '$remarks', DocStatus = '$doc' WHERE Channel = 'PROCUREMENT' AND Reference = '$reference' AND DocInOut = 'OUT'";
    $result = $conn->query($updateQuery);

    $updateQuery = "UPDATE tbl_inout SET ActionTaken = '$action', Remarks = '$remarks', DocStatus = CASE 
    WHEN Channel = 'PROCUREMENT' AND DocInOut = 'OUT' THEN 'RELEASED'
    ELSE '-'
  END 
  ...
  $results = $conn->query($updateQuery);
    ...
    if ($result) {
        if ($doc === 'COMPLETED') {
            // Send an email
            $sql = "SELECT * FROM tbl_inout WHERE Channel = 'PROCUREMENT' AND Reference = '$reference' AND DocInOut = 'OUT'";
        ...
        if ($doc === 'RELEASED') {
            ...
            elseif ($action === 'DISAPPROVED') {
              ...
              $updateQuery = "UPDATE tbl_inout SET ActionTaken = '$action', Remarks = '$remarks', DocStatus = CASE 
              WHEN Channel = 'PROCUREMENT' AND DocInOut = 'IN' THEN 'DISAPPROVED'
              ELSE 'NULL'
            END 
