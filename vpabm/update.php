...
echo'
    <div class="mx-4 px-2 pb-2">
        <div class="title">
          ...
          $query = "SELECT * FROM tbl_inout WHERE Channel = 'VPABM' AND DocStatus = 'RECEIVED' ORDER BY CDate DESC";
  ...
  if (isset($_FILES["new_upload"]["name"])) {
    if (isset($_FILES["new_upload"]) && $_FILES["new_upload"]["error"] == UPLOAD_ERR_OK) {
      ...
      if (move_uploaded_file($_FILES["new_upload"]["tmp_name"], $uploadFile)) {
          ...
          $updateQuery = "UPDATE tbl_inout SET ActionTaken = '$action', Remarks = '$remarks', DocStatus = '$doc', new_upload = '$uploadValue' WHERE Channel = 'VPABM' AND Reference = '$reference' AND DocInOut = 'OUT'";
